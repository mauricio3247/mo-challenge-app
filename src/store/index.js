...
export default new Vuex.Store({
  state: {
    ...
    checkOut: moment().add(1, 'days').format(formatDate),
    adults: 1,
    childrens: 0,
    pets: false,
    home: null,
  ...
  getters: {
    ...
    getNightsCount (state) {
      if (state.checkIn === null || state.checkOut === null) {
        ...
        return moment (state.checkOut, formatDate).diff (moment (state.checkIn, formatDate), 'days')
      }
    },
  ...
  mutations: {
    ...
    CLEAR_STATE (state) {
      ...
      state.checkOut = moment().add(1, 'days').format(formatDate)
      state.adults =1
      state.childrens = 0
      state.messageHost = ''
    },
  ...
  actions: {
    async load ({commit}) {
      try {
        commit('CLEAR_STATE')
        const response = await fetch(`${URL_BACKEND}/api/listings/28eed9aa-c27d-4217-ab21-ad65ead3a2aa`);
